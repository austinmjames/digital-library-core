...
// Using robust absolute paths to resolve module resolution issues
 ...
 * Specialized discovery logic for Sovereignty community layers.
 */
export function MarketplaceView({ onSelect }: MarketplaceViewProps) {
  ...
  const handleInstall = async (id: string) => {
    try {
      ...
      loadMarketplace(); // Refresh to trigger exclusion rule (hiding installed)
    } catch (err) {
  ...
  const filteredItems = useMemo(() => {
    return items.filter((item: MarketplaceVersion) => {
      const matchesSearch =
        ...
        {filteredItems.length === 0 ? (
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              {filteredItems.map((item: MarketplaceVersion) => (
                <MarketplaceItemUI
                  key={item.id}
                  item={item}
                  onSelect={onSelect}
